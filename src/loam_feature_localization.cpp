...
//}
  ...
  void LoamFeatureLocalization::extractNearby()
  {
    ...
    kdtreeSurroundingKeyPoses->radiusSearch(cloudKeyPoses3D->back(), (double)surrounding_key_frame_search_radius_, pointSearchInd, pointSearchSqDis);
  ...
  void LoamFeatureLocalization::extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
  {
    ...
    {
      if (utils->pointDistance(cloudToExtract->points[i], cloudKeyPoses3D->back()) > surrounding_key_frame_search_radius_)
        continue;
  ...
  pcl::PointCloud<PointType>::Ptr LoamFeatureLocalization::transformPointCloud(pcl::PointCloud<PointType>::Ptr cloudIn, PointTypePose* transformIn)
  {
  ...
  void LoamFeatureLocalization::scan2MapOptimization()
  {
    ...
    if (laserCloudCornerLastDSNum > edge_feature_min_valid_num_ && laserCloudSurfLastDSNum > surf_feature_min_valid_num_)
    {
